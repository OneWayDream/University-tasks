...
void task_1() {
	...
	//ѕор€док вывода команд произволен - у потоков нет чЄткой последовательности выполнени€ - это случайность.
}
...
void task_2(int threads_amount) {
	...
	//ѕрограмма работает корректно, но количество потоков, которые в итоге задействованы - 16 (дл€ мен€).
	//ћожно указать их точное количество. ¬ходной параметр вли€ет лишь на то, будет ли распараллеливание или нет
}
...
void task_6(int a[], int n) {
	...
	// –азница результатов - в простом цикле могут происходить потоковые ошибки. ј reduction убирает их.
}
